  ...
  geom_jitter(aes(x=tau, y=0, color=factor(J)), 
              data=comp_risk_delta_period %>% 
                mutate(tau = pmax(0, T-V_early),
                       J = factor(J, levels=c(0,1), labels=c("Vaccine-Irrelevant", "COVID-19"))), 
...
grid<-cbind(
  ...
  #c(rep(0, 14), rep(1, max_tau_delta+1)),
  rbind(matrix(0, nrow=length(times_pre), ncol=ncol(sieve_omicron$basis)),
  ...
  geom_jitter(aes(x=tau, y=0, color=factor(J)), 
              data=comp_risk %>% 
                ...
                mutate(tau = pmax(0, T-V_early),
                       ...
                       J = factor(J, levels=c(0,2), labels=c("Vaccine-Irrelevant", "COVID-19"))) %>%
